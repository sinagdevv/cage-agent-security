...
from app.schemas.enums import IntentStatus, PolicyDecision
...
class IntentRulesEvaluator:
    ...
    def evaluate_intent(
        ...
        if contract.agent_id != action.agent_id or contract.session_id != action.session_id:
            return [
                MatchedRule(
                    rule_id="RULE_INTENT_MISMATCH",
                    decision=PolicyDecision.DENY,
        ...
        unauthorized_data = [
            c for c in effective_classifications if c not in contract.allowed_data_classifications
        ]
